console.log("✅ emotional.js is loaded");
...
function updateGlobalProgressBar() {
  ...
  ["behavioral", "emotional", "demographic"].forEach((category) => {
    ...
    if (category !== "emotional" && data[category]) {
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const data = JSON.parse(localStorage.getItem("selectedAffirmations")) || {};
  delete data.emotional; // ✅ Fixed: delete current page data, not emotional
  ...
  window.allAffirmations = {
    celebration: affirmationsData.emotional.celebration,
    transformation: affirmationsData.emotional.transformation,
  };
  ...
  selected[type] = { trait, text, highlight };
    ...
    allSelections.emotional = selected;
    localStorage.setItem("selectedAffirmations", JSON.stringify(allSelections));

    console.log("🚀 Navigating to demographic.html");
