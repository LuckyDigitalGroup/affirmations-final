console.log("✅ demographic.js is loaded");
...
function updateGlobalProgressBar() {
  ...
  ["behavioral", "emotional", "demographic"].forEach((category) => {
    ...
    if (category !== "demographic" && data[category]) {
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const data = JSON.parse(localStorage.getItem("selectedAffirmations")) || {};
  delete data.demographic; // ✅ Fixed: delete current page data, not emotional
  ...
  window.allAffirmations = {
    celebration: affirmationsData.demographic.celebration,
    transformation: affirmationsData.demographic.transformation,
  };
  ...
  selected[type] = { trait, text, highlight };
    ...
    allSelections.demographic = selected;
    localStorage.setItem("selectedAffirmations", JSON.stringify(allSelections));

    console.log("🚀 Navigating to affirmations.html");
